from guanwang.models import Menus, MenuItem
...
class SubMenuObject(template.Node):
    ...
    def render(self, context):
        ...
        user = context['request'].user
        menu = False
        for m in Menus.objects.filter(base_url__isnull=False):
...
def get_items(menu_name, current_path, user):
    ...
    menu = Menus.objects.filter(slug=menu_name).first()
    ...
    for i in MenuItem.objects.filter(menu=menu).order_by('position'):
        ...
        if (not (i.login_required or i.anonymous_only)) or (i.login_required and show_auth) or (
            i.anonymous_only and show_anonymous):
